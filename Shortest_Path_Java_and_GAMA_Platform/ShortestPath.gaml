   ...
   aspect base {
		...
		if (source_consumer=true){
	     ...
	     if (flag=false and Visit=true){
	     	draw string("Visited") color: #lightgreen size: 4 at: {self.location.x-1, self.location.y+2};
	     	if (length(list_all_nodes)=1){
	     		draw string("Visited") color: #lightgreen size: 4 at: {source.location.x-1, source.location.y+2};
	     	}	
	     }
	    
	     draw string(self) color: #white size: 4 at: {self.location.x, self.location.y};	
      	}//aspect
}//end builtin node 
   ...
   aspect base {
   		 ...
   		 if layout!="Grid"{
	   		 if (weighted=false){
	   			...
	   			draw string((g_graph weight_of self) with_precision 1) color: #white size: 4 at: {self.location.x, self.location.y+2};	
	   		}
   		}
    }
}
...
experiment Graph type: gui  {
	...
	output{
		...
		display Graph background:rgb(10,40,55) {
        ...
        graphics "path" { 	
        	if layout="Grid"{
	        	loop v over: g_graph.vertices {
					draw circle(1) at: geometry(v).location color: #orange;
					draw string(v) color: #white size: 4 at: {geometry(v).location.x,geometry(v).location.y};	
				}
				loop e over: g_graph.edges {
				}
			}
        	if (shortest_path != nil) {
	            draw circle(1.5) at: source color: #red border: #orange;
	          	draw circle(1.5) at: target color: #yellow border: #orange;
				draw (shortest_path.shape + 0.2) color: #yellow;
			}
		}
        }
 	}
